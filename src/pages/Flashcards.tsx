    ...
    <div className="relative flex flex-col items-center justify-center min-h-screen gap-8 px-4 text-center">
      ...
      <div>
        ...
        <p className="mt-3 text-muted-foreground">
          Você revisou <span className="text-success font-bold">{revisados}</span> flashcard{revisados !== 1 ? "s" : ""}.
        </p>
      </div>
...
const FlashcardSession = ({
  ...
  const handleFlip = useCallback(() => {
    if (!transitioning) setFlipped((p) => !p);
  }, [transitioning]);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="relative mx-auto max-w-xl px-4 pt-24 pb-16">
        ...
        <div className={`transition-all duration-500 ${flipped ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}>
          <p className="text-center text-xs text-muted-foreground mb-3 font-medium uppercase tracking-wider">Como foi sua resposta?</p>
          <div className="flex gap-3">
      ...
      <main className="relative mx-auto max-w-4xl px-4 pt-24 pb-16">
        ...
        <div className={`mb-8 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          ...
          <p className="mt-2 text-muted-foreground">
            {totalPendentes > 0
              ? <><span className="text-yellow-400 font-semibold">{totalPendentes}</span> flashcard{totalPendentes !== 1 ? "s" : ""} pendente{totalPendentes !== 1 ? "s" : ""} de revisão</>
              : "Todos os flashcards em dia! 🎉"}
          </p>
        </div>
